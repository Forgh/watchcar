...
const gchar * serverIpAddress = "FIXME";
...
static void *app_function (void *userdata) {
  ...
  pipeline_definition = malloc(109); //Number of characters in the pipeline definition
  ...
  const gchar *  part2 = " port=5000 ! gdpdepay ! rtph264depay ! ffdec_h264 ! autovideosink sync=false";
...
/*static void native_Set_Ip_server (JNIEnv *env, jobject thiz, jstring ipAddress) {
	CustomData *data = GET_CUSTOM_DATA (env, thiz, custom_data_field_id);
	if (!data) return;
